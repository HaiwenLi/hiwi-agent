...
export function isKeyRelease(data: string): boolean {
		...
		// Kitty keyboard protocol release events (flag 2):
		//   \x1b[<codepoint>;<modifier>:3u
		// Match only when the pattern appears as part of a CSI escape sequence
		// to avoid false positives on plain text containing substrings like ":3u".
		const KITTY_RELEASE = /\x1b\[\d+;\d*:3[u~A-HF]/;
		return KITTY_RELEASE.test(data);
	}
