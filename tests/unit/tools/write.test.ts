...
describe("write_file tool", () => {
  ...
  it("creates parent directories if missing", async () => {
    ...
    const result = await tool.execute({ path: filePath, content: "nested" }, ctx);
    expect(result.isError).toBe(false);
    const written = await fs.readFile(filePath, "utf-8");
  ...
  it("handles relative paths against workingDirectory", async () => {
    const result = await tool.execute({ path: "relative.txt", content: "rel" }, ctx);
    expect(result.isError).toBe(false);
    const written = await fs.readFile(path.join(tempDir, "relative.txt"), "utf-8");
    expect(written).toBe("rel");
  });

  it("returns error when content is missing", async () => {
    const result = await tool.execute({ path: path.join(tempDir, "fail.txt") }, ctx);
  ...
  it("returns title with file path", async () => {
    ...
    const result = await tool.execute({ path: filePath, content: "t" }, ctx);
